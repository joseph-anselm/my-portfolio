  ...
  return (
    <Layout seo={generateBlogPostSEO(post)}>
      <article className="min-h-screen bg-white">
        ...
        {/* Top Nav */}
        <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b px-6 py-4">
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs text-slate-400 font-bold uppercase">
              <Link href="/">Home</Link>
              <ChevronRight size={12} />
        ...
        {/* Header */}
        <header className="text-center py-16 px-6 max-w-4xl mx-auto">
          <Link href="/blog">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-600 text-xs font-bold uppercase">
              <Tag size={12} />
              {post.categoryName}
            </span>
          </Link>
          ...
          <div className="mt-6 flex justify-center items-center gap-4 text-sm text-slate-500">
            <User size={16} />
  ...
  try {
    const post = await sanityClient.fetch(
      `*[_type == "blogPost" && slug.current == $slug][0]{
        ...
        "featuredImageUrl": coalesce(featuredImage.asset->url, "/images/fallback-blog.jpg"),
    ...
    const reactionCounts = await sanityClient
      .fetch(
        `{
      ...
      .catch(() => ({ likes: 0, dislikes: 0 }));

    return {
      props: {
        post: { ...post, reactionCounts },
      },
      revalidate: 60,
    };
  } catch (err) {
